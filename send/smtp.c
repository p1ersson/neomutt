...
struct SmtpAuth
{
   ...
   */
  int (*authenticate)(struct SmtpAccountData *adata);
...
static int smtp_auth_sasl(struct SmtpAccountData *adata)
{
  ...
  const char *mechlist = adata->auth_mechs;
...
static const struct SmtpAuth smtp_authenticators[] = {
  { smtp_auth_oauth, "oauthbearer" }, { smtp_auth_plain, "plain" },
#ifdef USE_SASL
  { smtp_auth_sasl, NULL },
#endif
};
